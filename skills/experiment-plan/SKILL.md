...
description: Design an efficient, claim-aware body of experiments for a serious method or causal thesis. Use when the researcher needs decisive comparisons, benchmark coverage, ablations, utility checks, run order, compute planning, and adaptive decision points without treating the plan as a rigid stage gate.
---
...
## Codex Plan Review At Real Forks

Use `research-review` before a claim-defining compute commitment when there is a real choice among causal tests, baselines, or method designs. Do not call it merely to bless a finished matrix.

Provide the original program, current paper bridge, thesis, primitive, scope debt, strongest alternative, forecasts, proposed experiment blocks, compute cost, and side-effect risks. Ask Codex whether perfect success is worth the cost, which experiment most changes the thesis or method, whether the plan returns to a standard surface, which planned block is ceremonial, and what decisive comparison is missing. Then ask it to attack confound parity and the weaker-story explanation.

Use a fresh thread for an independent plan review. Use `mcp__codex__codex-reply` only to revisit the same plan after supplying corrected assumptions or new pilot evidence. Record the adopted and rejected recommendations in the plan rationale.

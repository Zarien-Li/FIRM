...
**In practice:** For a typical ML paper, this skill handles ~60% of figures (all data plots + tables). The remaining ~40% (hero figure, architecture diagram, qualitative results) need to be created manually and placed in `figures/` before running `/paper-writing`. The skill will detect these as "existing figures" and preserve them.
...
- **REVIEWER_MODEL = `optional-account-default`** — Reserve one optional Codex figure
  audit for a near-final paper when a named claim or readability risk remains.

## Inputs

1. **PAPER_PLAN.md** — figure plan table (from `/paper-writing`)
...
- The single scientific question and claim-level job of each figure/table
- Whether the figure establishes motivation, mechanism, main effect, necessity, boundary, cost, or scope
...
### Step 7: Figure Quality Review

Visually inspect rendered figures against the claims-evidence spine. Do not call Codex
for routine figure iteration. If the paper is near final and one named claim or
readability risk remains, an optional account-default Codex audit may inspect the full
figure set once:
...
mcp__codex__codex:
  ...
  prompt: |
    ...
    6. What exact paper claim does this figure support, and is that claim within the data scope?
    7. Does this figure advance the paper's central spine or merely preserve research history?
...
- [ ] Figure/table has one stated rhetorical job and a mapped paper claim
- [ ] Caption distinguishes natural vs induced, existence vs prevalence, and setting-specific vs general evidence when relevant
- [ ] No figure introduces a claim that is absent from the claims-evidence matrix
- [ ] Figure 1 makes the problem and intervention locus legible without pretending to be empirical proof
...
- **Figures are argument units, not result storage** — omit panels that do not change a claim, explanation, boundary, or reviewer decision
- **Do not make scope visual ambiguity do rhetorical work** — label induced settings, oracle results, single-cell evidence, and unavailable prevalence directly

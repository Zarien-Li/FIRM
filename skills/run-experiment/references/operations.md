# Run Experiment Operations Reference

This is the preserved detailed operations manual for `run-experiment`. Read only the
section required by the project's actual allocation model: local, direct-GPU SSH,
Slurm, W&B, or batch queue. Scientific justification and current core rules live in
`../SKILL.md`; if this historical manual repeats them, the core skill wins.
...
Identify the one authoritative live-state file. If the state, task list, memory,
experiment tracker, or manuscript disagree about the active method, optimizer,
evaluator, decisive baseline, run status, or next action, reconcile them from code
and raw artifacts before launch. A task marked `in_progress` is not scientific
authority and must not resurrect a held method or stale paper identity.
...
- coding and a labeled smoke/activation pilot may precede complete comparison coverage,
  but a full run intended to support or reject the method includes a competent
  decisive incumbent and strongest simple alternative, or runs them under matched
  treatment in the same campaign
- the live state records the current transition note (`PRIZE`, `FIDELITY`,
  `INCUMBENT`, `DESIGN`, `DECISION`) when this is the first full method comparison or
  follows a material task, population, metric, primitive, or paper-identity change
...
- the run fits the authorized operational budget envelope and records its expected charge; only spend outside that envelope or above its escalation threshold requires fresh user approval
...
### Contract Fidelity, Training Health, And System Profile

For contracted method runs, read `docs/research_contract.md` before implementation or launch and cross-check:

- the code matches the contracted intervention;
- metrics match the contract's success/failure signals;
- outputs are machine-readable;
- baseline path remains comparable;
- no uncontracted variation, silent scope change, or fallback condition has entered the run.

If the implementation deviates materially from the contract, stop and update `research-contract` before launching. Environment, path, and logging fixes are allowed; changes to stopping rules, thresholds, objectives, data filtering, inference rules, or metric definitions require re-registration or a baseline semantics review.

Before any main run, verify upstream legitimacy: the anchor comparison exists, baseline health is not `fragile`/`invalid`, benchmark framing is not ad hoc/circular/label-degenerate, and the method script was not allowed to outrun the evidence anchor. If the upstream state is invalid, the valid next action is baseline repair, benchmark rethink, or contract revision, not launch.

Before launch, perform an eval health check and record the exact evaluation script, metric entrypoint, model/checkpoint path, run scope (`debug-only`, `pilot`, `sample`, or `full-eval`), and whether the benchmark is the real target benchmark or a synthetic/debug proxy. Define the evaluation population independently of the method: inclusion/exclusion rules, sampling frame, weighting, unit of analysis, and relation to the claimed population. Record each system's training treatment: data exposure, labels/supervision, objective, curriculum, optimization, adaptation, and compute. A difference in either population or treatment must be explicit and cannot be attributed to the method primitive without an appropriate control. If the eval path is still under active debugging, the run is `debug-only` or `pilot`, not paper evidence.

For every key derived metric, freeze and test its computation before scale: raw
source fields, numerator, denominator, units, aggregation order, direction,
missing/invalid handling, weighting, and any clipping or normalization. Recompute at
least one representative result independently from raw outputs or sufficient
statistics and compare within a declared tolerance. A parser-level smoke success or
plausible headline value is not this validation.

Fold training health checks and system profiling into the owning run instead of opening a separate profiling workflow. Check environment, dependency path, data path, GPU/scheduler allocation model, memory bottleneck, throughput, disk space, checkpoint write path, and log/result parseability. Use these checks to repair execution, not to reinterpret broken runs as scientific evidence.

When implementation fails repeatedly, record root cause before relaunching: observed failure, root cause, files changed, why the change fixes the cause rather than hiding the symptom, and which previous runs or claims are invalidated. If root cause is unknown, the next run is `debug-only`, not a main experiment.

One verified implementation, optimizer, evaluator, or integration defect normally
earns one clean matched rerun. If the rerun preserves the directional failure, do
not launch another nearby version unless the authoritative state names a changed
load-bearing assumption and a prediction that distinguishes it. A new run name,
coefficient, seed, threshold, or auxiliary penalty is not a new design.
...
- a lifecycle record whose status moves `planned -> queued/running ->
  completed_unread`; launch code MUST NOT write `interpreted`
- an operational-budget record: authorized envelope source, estimated and actual
  charge when measurable, remaining envelope, and escalation threshold
...
`completed` is an execution status. Each job record in `queue_state.json` must also
carry a scientific handoff status:

```json
{
  "id": "job-id",
  "status": "completed",
  "analysis_status": "unread",
  "result_path": "...",
  "log_path": "...",
  "checkpoint_path": "...",
  "completion_marker": "...",
...
Allowed `analysis_status` values are `unread | interpreting | interpreted |
deferred | invalidated`. The queue manager initializes successful jobs as `unread`;
only `signal-analysis` or an explicit evidence-reading step may set `interpreted`.
State writes must be atomic (temporary file plus rename) so monitoring never reads a
...
#### Step B3: Launch Scheduler

The scheduler implementation lives in `tools/experiment_queue/queue_manager.py`. Three preliminaries before launch.

**B3a. Resolve the local helper directory.** The two helpers (`queue_manager.py`, `build_manifest.py`) sit under `tools/experiment_queue/` in the ARIS repo. Use this fallback chain so the skill works from any project layout:
...
**B3b. Compute remote paths.** Use both a remote-relative form (for `scp` destinations — modern `scp` runs in SFTP mode and does NOT reliably expand `$HOME` in destination paths) and a `$HOME`-prefixed form (for `ssh ... command` strings, where remote bash WILL expand `$HOME`):
...
**B3c. Bootstrap the remote run directory and copy helpers + manifest.** Per-invocation and idempotent. Use a unique run directory rather than `/tmp` so concurrent queues do not collide and so resume-after-crash is reproducible.
...
**B3d. Launch the scheduler as a detached `nohup` process on the SSH host:**
...
**B3e. Resume an existing queue (only when the user asks).** A fresh `RUN_TS` per invocation is correct for *new* queues. To resume a crashed queue, do NOT regenerate `RUN_TS` — reload the recorded values and re-run only the launch command (Step B3d), not the bootstrap (Step B3c):
...
# Then re-run Step B3d verbatim. Do NOT re-run Step B3c (would overwrite manifest.json + state.json).
```
...
User can check state anytime, using `$REMOTE_RUN_DIR` from Step B3b (or reload from `$LOCAL_RUN_DIR/run_meta.txt` for an older run):
...
`/monitor-experiment` reads `queue_state.json` directly when this path exists. Use
the literal command above only as a manual diagnostic fallback.
...
- Monitoring reconciles terminal jobs and hands every valid
  `completed + analysis_status=unread` result to `signal-analysis`.
- The run/queue becomes lifecycle-complete only after the returned interpretation
  patch is merged into the authoritative research state; `stuck`, `invalidated`, or
  deferred cells remain explicit.
...
Scheduler implementation references: `tools/experiment_queue/queue_manager.py` and `tools/experiment_queue/build_manifest.py` (resolved at runtime via the fallback chain in Step B3a).
...
- Do not let active debugging, system profiling, or a successful smoke test become evidence for the method; label such runs `debug-only` or `pilot` and require a clean rerun before claim use
- A smoke test proves only that the selected path executes and emits inspectable
  output. It does not prove semantic equivalence to the reference baseline,
  evaluator, training treatment, data population, stopping rule, or metric.
- Validate key derived metrics against raw outputs or sufficient statistics before
  using them for a scientific decision; record any discrepancy and invalidate
  dependent results until repaired.
- Reallocate jobs autonomously inside the authorized operational budget. Ask the
  user only before exceeding its compute/time/API/storage envelope or changing a
  user-locked resource rule.

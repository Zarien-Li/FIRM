...
description: Persistent lead-researcher and first-author posture for an empirical research program. Preserve the important program, build field intuition, maintain competing explanations, cultivate a method through coherent research episodes, expand credible positive principles, and harvest papers without turning research into a gate-driven audit pipeline.
---
...
Use `frontier-direction-discovery` when the broad program itself is missing or being
refreshed. Use `research-lit` and `baseline` for field contact, nearest rivals,
...
paper identity. Use `signal-analysis` when a bundle can distinguish predictions or
...
Use `method-primitive-synthesis` when contact leaves a valuable design opportunity.
...
Use `paper-writing` for paper formation and writing. Submission-oriented expansion and
readiness require the later evidence synthesis defined there; an optional sparse
independent audit verifies a mature object but does not create maturity. Early notes
and research drafts never confer maturity.

## Use Independent Colleagues Without Creating Gates

Use `research-review` with one role matched to the episode. These are optional
collaborations, not pipeline stations:
...
`research-review` for the verified invocation and model pin; do not silently fall back
...
synthesis and action. When feasible, let the smallest real experiment criticize an
idea before Codex does.
...
claims that depend on it and is propagated through `evidence-lineage.md`; repair,
...
cases under `tests/regression-scenarios.md` preserve accumulated lessons. They are not

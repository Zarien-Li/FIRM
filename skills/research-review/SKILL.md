...
Ask only what the bundle distinguishes, what remains unidentified, and whether a
paper-critical inference exceeds the evidence. It does not invent a method or decide
...
The lead PI, optionally with Gemini, owns consolidation and re-grounding after a
completed episode. Do not call Codex by default. Separate:
...
External criticism must not invent a hidden variable, deeper abstraction, new method,
diagnostic taxonomy, analysis-paper identity, project-level `PARK`, or broad closure.
Same-arc continuation needs a genuinely new design question supported by evidence,
not a new name or finer private cell. There is no fixed failure count.
...
construction arc. Operational repair and waiting are not review events. A repeated
call needs corrected facts, materially new evidence, or a genuinely different
question; never shop for agreement.
...
For Codex, call `mcp__codex__codex` and `mcp__codex__codex-reply` without a `model`
field. Retry one identical transient capacity failure.

...
description: "Autonomously improve a generated paper via account-default Codex review → implement fixes → recompile, for 2 rounds. Use when user says \"改论文\", \"improve paper\", \"论文润色循环\", \"auto improve\", or wants to iteratively polish a generated paper."
...
This skill is designed to run **after** Workflow 3 (`/paper-writing` — plan → figures → LaTeX → compile). It takes a compiled paper and iteratively improves it through external LLM review.

Unlike `/research-review` Looped Mode (which iterates on **research** — running experiments, collecting data, rewriting narrative), this skill iterates on **paper writing quality** — fixing theoretical inconsistencies, softening overclaims, adding missing content, and improving presentation.

When `writing_models/SYNTHESIS.md` exists, the fix implementer may use it as author-side structural guidance. Do not reopen all reference PDFs in the main thread. If a major structural comparison is needed, delegate the full-paper reread to a fresh subagent and consume only its compact report. Never give the synthesis, reference PDFs, or prior exemplar judgments to the independent Codex reviewer.

## Constants

- **MAX_ROUNDS = 2** — Two rounds of review→fix→recompile by default. Add another round only when the latest review exposes a new correctness or structure issue that materially changes the manuscript.
- **REVIEWER_MODEL = `account-default`** — Use Codex MCP without a `model` field; inherit the current ChatGPT-account reviewer model per `/research-review`.
...
Only when `--style-ref: <source>` appears in `$ARGUMENTS`, derive a compact structural
profile before the loop starts. If the host repository provides an
`extract_paper_style.py` helper, it may be used. Otherwise delegate the source-paper reading to
an isolated subagent and consume only a compact report covering section jobs, argument order,
figure density, and prose cadence. Missing optional tooling must not abort ordinary paper
improvement.
...
- **Never pass `--style-ref` (or the cache contents) to the Codex reviewer.** The Reviewer Independence Protocol below requires reviewers see only the artifact and the user's prompt — leaking the style ref would contaminate the review with author-side context. **This is the most critical invariant in this skill.**
...
This is the parameter that upstream pipelines use to enforce text-only microedits: no `.bib` mutations, no `.sty` / `.bst` mutations, no edits to prior-submission directories, no new `\cite{...}`, no new theorem environments, no new numerical claims.
...
```bash
# Text-only mode via edit whitelist:
...
Send the full paper text AND compiled PDF to the account-default Codex reviewer with maximum available reasoning:
...
Parse user response same as `/research-review` Looped Mode: approve / custom instructions / skip / stop.
...
| Proof gap (theory papers) | Run `/research-review` as a proof audit if PROOF_AUDIT.md doesn't exist yet; fix FATAL/CRITICAL issues |
| Writing clutter / passive voice | Apply a five-pass prose audit: clutter extraction → active voice → sentence architecture → keyword consistency → numerical integrity. See `paper-writing` |
| Number mismatch (paper vs results) | Run the claim audit (`/paper-writing` Phase 4.7) if PAPER_CLAIM_AUDIT.md doesn't exist; fix any `number_mismatch` or `aggregation_mismatch` claims |
...
**Optional deeper check: `/research-review` restatement audit**

The inline Python check above is the default and is sufficient for routine main-vs-appendix consistency. For broader coverage, you may additionally invoke `/research-review` (fresh thread, maximum reasoning) as a semantic restatement audit, asking the reviewer to extend the comparison to:

- Restatements in summary tables, abstract, "Key Contributions" lists, and discussion sections (not just main vs appendix).
- Named drift signatures classified by reviewer: `conditional_loss`, `scope_change`, `quantifier_loss`, `regime_envelope_change`, `constant_change`, `variable_rename`.
- Structured findings recorded in the review synthesis, suitable for downstream audit trails.

This is advisory only — the inline Step 4.5 check remains the default and continues to run on every loop round. Consider the deeper review when (a) you suspect cross-location drift outside the main↔appendix axis (e.g., abstract overclaim relative to theorem statement), or (b) you want reviewer-graded drift signatures rather than raw string mismatches. Running both is supported and they are independent: the inline check fails fast on string drift, the review pass surfaces semantic-class drift.
...
<details>
<summary>Codex account-default Review (Round 1)</summary>
...
<details>
<summary>Codex account-default Review (Round 2)</summary>
...
- **Save FULL raw review text** — do not summarize or truncate Codex responses
- **Prose structure discipline** — when implementing fixes, do not add excessive bold, decorative contribution labels, unnecessary `itemize`, one-or-two-sentence paragraph fragmentation, or tiny subsections/subsubsections. Prefer cohesive paragraphs and headings only for real conceptual units. If a reviewer asks for clarity, first improve the prose and transitions; do not automatically turn the paper into a checklist.
- **Protect the paper spine** — a reviewer fix may narrow, clarify, or better support the contribution, but should not make a secondary result, late ablation, or fashionable related paper take over the narrative unless it genuinely changes the bounded claim.
- **Keep section jobs distinct** — abstract is one compact argument; introduction motivates and positions without duplicating equations; method explains the design; experiments answer claim-level questions; related work uses field-facing categories; limitations state unsupported boundaries rather than hiding result-changing negatives.
- **Writing cannot repair missing science** — if a proposed fix requires a new empirical claim, new generality, natural prevalence, or a stronger causal conclusion, record it as a research gap instead of manufacturing prose.
- **No strawman fixes** — do not improve the paper by weakening related work, baselines, reviewer objections, negative results, or limitations. If a fix criticizes an incumbent or objection, first state the strongest faithful version and then differentiate against that. Reject edits that make the paper stronger only by making the opponent weaker.
...
## Interpreting Score Progression

Reviewer scores are diagnostic, not optimization targets. In one end-to-end development run,
successive rounds moved from major structural problems to narrower presentation issues, but that
trajectory is an anecdote rather than an expected gain:
...
| Round 3 | format-only follow-up | Fixed venue-length and layout problems |

Do not claim a typical numerical improvement from this table. A lower later score can be more
useful if the fresh reviewer detects an earlier blind spot.

## Review Tracing

After each reviewer call, save the trace following `shared-references/review-tracing.md`.
Write it directly to `.research-skills/traces/<skill>/<date>_run<NN>/`; an optional local
trace helper may automate this, but the skill must not depend on an unpublished script.
